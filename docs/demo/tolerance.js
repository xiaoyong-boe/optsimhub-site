...
const TMMCalc = {
    ...
    tmm(pol, n_list, d_list, theta_deg, wl_nm) {
        ...
        for (let i = 1; i < n_list.length - 1; i++) {
            ...
            // Pre-multiply: M = L @ M_prev  (matches Python convention)
            if (i === 1) {
                M = L;
            } else {
                // L @ M: complex 2×2 matrix multiplication
                const l = L, m = M;
                // C00 = L00*M00 + L01*M10
                const c00_re = l[0][0]*m[0][0] - l[0][1]*m[0][1] + l[0][2]*m[1][0] - l[0][3]*m[1][1];
                const c00_im = l[0][0]*m[0][1] + l[0][1]*m[0][0] + l[0][2]*m[1][1] + l[0][3]*m[1][0];
                // C01 = L00*M01 + L01*M11
                const c01_re = l[0][0]*m[0][2] - l[0][1]*m[0][3] + l[0][2]*m[1][2] - l[0][3]*m[1][3];
                const c01_im = l[0][0]*m[0][3] + l[0][1]*m[0][2] + l[0][2]*m[1][3] + l[0][3]*m[1][2];
                // C10 = L10*M00 + L11*M10
                const c10_re = l[1][0]*m[0][0] - l[1][1]*m[0][1] + l[1][2]*m[1][0] - l[1][3]*m[1][1];
                const c10_im = l[1][0]*m[0][1] + l[1][1]*m[0][0] + l[1][2]*m[1][1] + l[1][3]*m[1][0];
                // C11 = L10*M01 + L11*M11
                const c11_re = l[1][0]*m[0][2] - l[1][1]*m[0][3] + l[1][2]*m[1][2] - l[1][3]*m[1][3];
                const c11_im = l[1][0]*m[0][3] + l[1][1]*m[0][2] + l[1][2]*m[1][3] + l[1][3]*m[1][2];
